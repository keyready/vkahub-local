...
func (e *EventRepositoryImpl) AddEvent(addEventReq request.AddEventReq) (httpCode int, err error) {
	...
	go func() {
		// eventId := strconv.Itoa(int(newEvent.ID))
		for _, user := range users {
			...
			// 	MailName: "MassMailing",
			// 	Receiver: user.Mail,
			// 	Msg: other.MailBody{
			// 		TypeMsg: "text/html",
			// 		Message: fmt.Sprintf("%s/events/%s", os.Getenv("BASE_URL"), eventId),
			// 	},
			// }

			// mailError := appmail.SendMail(mailData)
			// if mailError != nil {
			// 	log.Println(mailError)
			// }
...
func (e *EventRepositoryImpl) ParseInfoEvent(url string) (httpCode int, err error, eventInfo string) {
	eventInfo, err = kagi.GetEventInfo(url)
	if err != nil {
		return http.StatusInternalServerError, err, ""
	}
	return http.StatusOK, nil, eventInfo
}
